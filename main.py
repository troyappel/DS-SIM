...
sort_pg = generate_programs.generate_mapreduce(25, 10, 1, 1, 1, 1, 1)
grep_pg = generate_programs.generate_mapreduce(25, 1, 1, 1, 1, 1, 1)
dask_pg = generate_programs.generate_dask()

datacenter_mg = generate_networks.generate_tree_machine_network(4,2,1,1,1,0.1,10, use_disk=True)

# Dump the output file here
outfilename = "generaltest.pickle"

# # Run without drawing the visualization, and write to a file
in_memory_history = Simulator(datacenter_mg, sort_pg).run(draw_visualization=False, outfile=outfilename)
...
print(history_df(in_memory_history).columns)

plot_mapreduce_data_transfer(history_df(in_memory_history))
